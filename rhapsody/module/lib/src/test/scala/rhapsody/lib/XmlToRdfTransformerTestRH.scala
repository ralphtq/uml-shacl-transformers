...
import rhapsody.lib.transformer.rdf.vocabulary.vocabulary.bindNamespaces

import java.io.FileInputStream
import java.nio.file.Paths
import scala.collection.JavaConverters._

class XmlToRdfTransformerTestRH extends AnyFlatSpec with should.Matchers {
...
// Tim's SPARQL queries accidentally pick up IClass's that are nested inside an _implicitClass
  ...
  List("Interface"
      ...
      , "ia_5_implicitType"
      , "ia_7_implicitType")
    .foreach(implicitClassName =>
      expectedUnionModel.listSubjectsWithProperty(RDFS.label, ResourceFactory.createPlainLiteral(implicitClassName))
        .toList
        .asScala
        .toList
        .map(implicitClassResource => {
